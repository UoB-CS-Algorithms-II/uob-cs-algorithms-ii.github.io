...
In slide 7 on multi-variable O-notation, in the example on f(m,n) &isin; O(g(m,n)), "f(m,n) &le; C &#xb7; mn" should read "f(m,n) &le; C &#xb7; g(m,n)".
...
In the correctness proof on slide 7, it's not true that V(T<sub>J</sub>) = V(S<sub>J</sub>). The rest of the proof still goes through as originally stated, but the current version of the slide is clearer than the one in the lecture video.
